/**
 * ANVESHA â€” Frontend Application Logic
...
async function uploadFile(file) {
    ...
    try {
        ...
        fill.style.width = '20%';
        status.textContent = 'File accepted â€” processing...';
...
function pollIngestionStatus(docId, filename, progress, statusEl, fill) {
    ...
    const poller = setInterval(async () => {
        ...
        if (pollCount > MAX_POLLS) {
            ...
            statusEl.textContent = 'âš  Processing is taking longer than expected. Check back later.';
        ...
        try {
            ...
            if (phase === 'complete') {
                ...
                statusEl.textContent = `âœ“ ${chunks} chunks extracted, ${entities} entities â€” running compliance debate...`;
            ...
            // Network error during poll â€” just retry on next interval
...
async function runUploadDebateAnalysis(docId, filename) {
    ...
    addMessage(`ðŸ“„ Analyzing compliance of uploaded document: ${filename}`, 'user');
    ...
    if (typingMsg) {
        typingMsg.querySelector('.typing-indicator').innerHTML = `
            <div style="display:flex;align-items:center;gap:10px;font-size:0.85rem;color:var(--text-secondary);">
                ...
                <span>ðŸ¤– Multi-Agent Debate â€” Advocate vs Skeptic vs Judge analyzing ${filename}...</span>
            </div>
        `;
    }
    ...
    try {
            ...
            reportMsg.innerHTML = `
                <div class="message-content" style="width:100%">
                    <div style="background:rgba(139,92,246,0.1);border:1px solid rgba(139,92,246,0.3);border-radius:12px;padding:16px;margin-top:8px">
                        <div style="font-size:0.85rem;font-weight:bold;color:#d0bcff;margin-bottom:12px">ðŸ“Š Compliance Analysis Report</div>
                        <div style="display:flex;gap:16px;flex-wrap:wrap;margin-bottom:12px">
                            ...
                            <div style="display:flex;flex-direction:column;gap:6px;flex:1">
                                <div style="display:flex;align-items:center;gap:8px">
                                    <span style="font-size:0.75rem;color:#4edea3;width:60px">âœ… MET</span>
                                ...
                                <div style="display:flex;align-items:center;gap:8px">
                                    <span style="font-size:0.75rem;color:#cebdff;width:60px">âš ï¸ PARTIAL</span>
                                    <div style="flex:1;height:6px;background:rgba(255,255,255,0.1);border-radius:3px;overflow:hidden">
                                ...
                                <div style="display:flex;align-items:center;gap:8px">
                                    <span style="font-size:0.75rem;color:#ffb4ab;width:60px">âŒ GAP</span>
                                    <div style="flex:1;height:6px;background:rgba(255,255,255,0.1);border-radius:3px;overflow:hidden">
                        ...
                        <div style="display:flex;gap:8px;flex-wrap:wrap">
                            <button onclick="switchMainView('compliance')" style="background:rgba(139,92,246,0.2);border:1px solid rgba(139,92,246,0.4);color:#d0bcff;border-radius:8px;padding:6px 14px;font-size:0.75rem;font-weight:bold;cursor:pointer">
                                ðŸ” View Full Compliance Matrix
                            </button>
                            <button onclick="downloadAnnotatedDocsForReport('${cr.report_id}')" style="background:rgba(78,222,163,0.1);border:1px solid rgba(78,222,163,0.3);color:#4edea3;border-radius:8px;padding:6px 14px;font-size:0.75rem;font-weight:bold;cursor:pointer">
                                ðŸ“„ Download Highlighted PDF Report
                            </button>
                        </div>
                    </div>
                </div>
        ...
        if (e.name === 'AbortError') {
            addMessage(`â±ï¸ Analysis timed out for ${filename}. The debate agents are taking too long. Try again or check API status.`, 'assistant');
        } else {
            addMessage(`âŒ Analysis failed for ${filename}: ${e.message}`, 'assistant');
        }
...
// Wrapper that uses the currently active report ID â€” called by matrixAnnotatedBtn
            ...
            return `
                <div class="doc-item" onclick="viewDocument('${doc.doc_id}')">
                    ...
                    <div class="doc-info">
                        ...
                        <div class="doc-meta">${doc.total_chunks} chunks â€¢ ${doc.content_type}</div>
                    </div>
                    <div class="doc-badge">${doc.status === 'success' ? 'âœ“' : 'âš '}</div>
                </div>
            `;
        }).join('');
    } catch (e) {
...
function getDocIcon(type) {
    ...
    if (['wav', 'mp3', 'm4a', 'ogg', 'flac', 'audio'].includes(type)) return 'ðŸŽµ';
    if (['png', 'jpg', 'jpeg', 'svg', 'image', 'schematic'].includes(type)) return 'ðŸ–¼ï¸';
    return 'ðŸ“Š';
}
...
function addAssistantMessage(text, confidence, citations, answerId, debateData = null) {
        ...
        const verdictBanner = `
            <div class="debate-verdict-banner ${verdictClass}">
                <div class="verdict-header-row">
                    <strong style="color:var(--text-primary); font-size:0.95rem;">âš–ï¸ Adjudicator Verdict</strong>
    ...
    const confLabel = confidence >= 70 ? 'ðŸŸ¢' : confidence >= 40 ? 'ðŸŸ¡' : 'ðŸ”´';
        ...
        hallucinationBlock = `
            <div style="margin-top: 1rem; border: 1px solid rgba(239, 68, 68, 0.3); border-radius: 8px; overflow: hidden; font-size: 11px;">
                <div style="background: rgba(239, 68, 68, 0.1); padding: 8px 12px; cursor: pointer; display: flex; justify-content: space-between; align-items: center; color: #ef4444; font-weight: bold;" onclick="toggleDebateCard('${detailsId}')">
                    <span>âš ï¸ Detected Hallucinations (${verification.rejected_claims.length})</span>
                    <span>â–¼</span>
                </div>
                <div id="${detailsId}" style="display: none; flex-direction: column; padding: 12px; background: rgba(31, 31, 34, 0.5); gap: 12px;">
                    ${verification.rejected_claims.map(c => `
                        <div style="border-left: 2px solid #ef4444; padding-left: 8px; margin-bottom: 8px;">
                            <div style="color: #ef4444; margin-bottom: 4px;"><strong>âŒ Hallucinated Claim:</strong> ${escapeHtml(c.text)}</div>
                            <div style="color: var(--text-secondary); margin-bottom: 4px;"><strong>ðŸ“ Reason:</strong> ${escapeHtml(c.reasoning || c.reject_reason || 'No specific reasoning provided')}</div>
                            ${c.evidence && c.evidence !== 'Not found in evidence' ? `<div style="color: #22c55e; margin-bottom: 4px;"><strong>âœ… Actual Evidence:</strong> <em>"${escapeHtml(c.evidence)}"</em></div>` : ''}
                            ${c.correction && c.correction !== 'N/A' && c.correction !== 'None' ? `<div style="color: #f59e0b; margin-bottom: 4px;"><strong>ðŸ”„ Correction:</strong> ${escapeHtml(c.correction)}</div>` : ''}
                        </div>
                    `).join('')}
                </div>
            </div>
    ...
    msg.innerHTML = `
        ...
        <div class="message-meta">
            ...
            ${answerId ? `<span style="cursor:pointer;color:var(--accent-cyan)" onclick="downloadEvidence('${answerId}')">ðŸ“¥ Evidence</span>` : ''}
        </div>
    `;
    container.appendChild(msg);
    container.scrollTop = container.scrollHeight;
    ...
    stageHeader.innerHTML = `
        <div class="message-content" style="width:100%;text-align:center">
            <div style="display:inline-flex;align-items:center;gap:8px;background:rgba(139,92,246,0.1);border:1px solid rgba(139,92,246,0.2);border-radius:20px;padding:6px 16px;font-size:0.78rem;color:#d0bcff;font-weight:bold">
                <span>âš”ï¸</span> Multi-Agent Compliance Debate <span>âš”ï¸</span>
            </div>
        </div>`;
    container.appendChild(stageHeader);
    
    // Advocate Message
    const t1 = addTypingIndicator('ðŸ’™ Advocate Agent analyzing evidence...');
    await new Promise(r => setTimeout(r, 2000));
    removeTypingIndicator(t1);
    await addAgentChatBubble('ðŸ’™ Advocate Agent â€” Pro-Compliance', advocate, 'advocate');
    
    // Skeptic Message
    const t2 = addTypingIndicator('ðŸ”´ Skeptic Agent challenging compliance...');
    await new Promise(r => setTimeout(r, 2000));
    removeTypingIndicator(t2);
    await addAgentChatBubble('ðŸ”´ Skeptic Agent â€” Counter-Argument', skeptic, 'skeptic');
    
    // Verdict Message
    const t3 = addTypingIndicator('âš–ï¸ Judge Adjudicator rendering verdict...');
...
function updateAnswerDetails(data) {
    ...
    el.innerHTML = `
        ...
        <div style="font-size:0.75rem; color:var(--text-secondary); margin-bottom:0.3rem">
            ${verification.abstained ? 'â›” System abstained â€” insufficient evidence' : ''}
        </div>
            ...
            if (historyList) {
                historyList.innerHTML = allReports.map((r, idx) => {
                    ...
                    const trend = idx < allReports.length - 1
                        ? (r.compliance_score > allReports[idx+1].compliance_score ? 'ðŸ“ˆ' : r.compliance_score < allReports[idx+1].compliance_score ? 'ðŸ“‰' : 'âž¡ï¸')
                        : '';
            ...
            <div class="space-y-3">
                ${report.controls.map((control, idx) => {
                    ...
                    const remediationHtml = control.remediation && control.remediation.length > 0
                        ...
                        `).join('')
                        : '<li>âœ“ No remediation required. Control fully satisfied.</li>';
                        ...
                        <div class="audit-item">
                            <div class="audit-item-header" onclick="toggleAuditDetails('${detailsId}')">
                                <div class="audit-item-title-block">
                                    ...
                                    <div class="audit-item-subtitle">${escapeHtml(control.framework)} â€¢ ${escapeHtml(control.category)}</div>
                                </div>
...
async function loadComplianceMatrix() {
    ...
    try {
        ...
        if (data.reports && data.reports.length > 0) {
            ...
            // No reports yet â€” show friendly prompt, do NOT auto-run slow audit
            grid.innerHTML = `
...
async function _refreshComplianceFromServer(grid) {
    try {
        ...
        // Silent background refresh â€” don't show errors
...
function applyComplianceFiltersAndRender() {
    ...
    grid.innerHTML = filtered.map((ctrl, idx) => {
        ...
        const remediationList = (ctrl.remediation && ctrl.remediation.length > 0)
            ? ctrl.remediation.map(rem => `<li class="text-on-surface/80 flex items-start gap-1.5"><span class="text-tertiary text-xs">â€¢</span><span>${escapeHtml(rem)}</span></li>`).join('')
            : '';
                ...
                <div class="flex flex-wrap items-start justify-between gap-2">
                    <div class="space-y-0.5">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-[11px] text-on-surface-variant">â€¢ ${escapeHtml(ctrl.category || '')}</span>
                        </div>
        ...
        listContainer.innerHTML = loadedReportsList.map((r, i) => `
            <div onclick="selectReport('${r.report_id}')" class="p-sm rounded-lg bg-surface-container-low border border-white/5 hover:border-primary/40 cursor-pointer transition-all ${i === 0 ? 'border-primary/40 bg-primary/5' : ''}">
                ...
                <div class="text-[10px] text-on-surface-variant flex justify-between mt-1">
                    ...
                    <span>${r.met_controls || 0} Met â€¢ ${r.gap_controls || 0} Gaps</span>
                </div>
            </div>
        `).join('');
        ...
        viewer.innerHTML = `
            <div class="space-y-lg max-w-4xl mx-auto">
                    ...
                    <div class="space-y-sm">
                        ${(report.controls || []).map(ctrl => `
                            <div class="p-md rounded-xl bg-surface-container-lowest border border-white/5 space-y-2">
                                ...
                                <div class="text-[10px] text-primary/70 font-semibold uppercase">${escapeHtml(ctrl.framework)} â€¢ ${escapeHtml(ctrl.category)}</div>
                ...
                <div class="pt-md border-t border-white/10 flex justify-between items-center">
                    <span class="text-[10px] text-on-surface-variant">Confidential Compliance Document â€¢ ANVESHA Engine</span>
...
function openCuratorTab() {
    switchMainView('dashboard');
    switchTab('curator');
    showToast('Graph Curator active â€” Add custom entities and relationships', 'info');
}
...
async function triggerLyzrRemediation(requirementId, name, description, reasoning) {
    ...
    const preLogSteps = [
        "ðŸ¤– Spawning Lyzr SecOps Agent (Instance: lyzr-secops-remediator-01)...",
        "âš™ï¸ Loading compliance metadata standards database...",
        "ðŸ”Œ Mapped control targets: " + requirementId,
        "ðŸ”Ž Querying Neo4j for surrounding entity relationships...",
        "ðŸ¤– Formulating remediation prompt plan..."
    ];
    ...
    writeLog("âš¡ Connection established with Lyzr Studio.", "success");
    ...
    try {
        ...
        const traceLogs = result.trace_logs || ["âœ“ Remediation task successfully completed."];
        ...
        writeLog("ðŸ Ready. Awaiting engineer execution approval...", "success");

    } catch (e) {
        if (lyzrLogInterval) clearInterval(lyzrLogInterval);
        writeLog(`âŒ Agent task crashed: ${e.message}`, "error");
...
async function executeLyzrPatch() {
    ...
    writeLog("ðŸš€ Running auto-remediation patch execution script...");
    writeLog("ðŸ“¦ Serializing code block: " + (lyzrActiveCode ? lyzrActiveCode.substring(0, 50) + "..." : "empty"));
    ...
    try {
        ...
        writeLog("âœ“ Local database / systems patched successfully.", "success");
        writeLog("âœ“ Neo4j Graph DB status updated. Relationship created.", "success");
        writeLog(`âœ“ Posture Compliance Score updated to ${data.compliance_score}%.`, "success");
        ...
        writeLog(`âŒ Execution crashed: ${e.message}`, "error");
...
async function sendComplianceConsultMessage() {
    ...
    try {
        ...
        appendConsultBubble("assistant", `âŒ Dialogue failed: ${e.message}`);
...
// ============================================================
// ============================================================
// ANVESHA â€” COMPREHENSIVE DEMO MODE (Apex_Security_Policy.pdf)
// Only activated when window.isDemoMode = true
// ============================================================

window.isDemoMode = false;

// â”€â”€â”€ Full hardcoded Apex Security Policy demo data â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    controls: [
        {
            ...
            name: "Article 32 â€” Security of Processing (Encryption)",
            ...
            evidence_found: [
                "Â§1.0: 'Apex Payments guarantees that 100% of customer data is encrypted in transit and at rest using AES-256'",
                "Â§3.0: 'The legacy transactions_db PostgreSQL database currently stores user credit card numbers and passwords in plain text due to reporting tool compatibility requirements'"
            ],
            reasoning: "CRITICAL CONTRADICTION DETECTED. Section 1.0 asserts 100% AES-256 encryption as a blanket guarantee. However, Section 3.0 explicitly admits that the legacy 'transactions_db' PostgreSQL database stores credit card Primary Account Numbers (PANs) and user passwords in plaintext. This constitutes a direct, documented violation of GDPR Article 32's requirement for encryption of personal data. The policy document itself has captured this contradiction in writing, which is a serious audit finding â€” the document is both making a compliance claim AND admitting to a violation of that same claim.",
            hallucination_flags: [
                { claim: "100% of customer data is encrypted in transit and at rest", verdict: "FALSE", color: "red", reason: "Contradicted by Â§3.0 admission of plaintext transactions_db storage. This is a documented lie within the same policy document.", page: "Section 1.0" }
            ],
            remediation: [
        ...
        },
        {
            ...
            name: "Control A.8.24 â€” Use of Cryptography",
            ...
            evidence_found: [
                "Â§1.0: 'AES-256 encryption standard mandated for all production databases'",
                "Â§4.0: 'Cryptographic key rotation is performed annually'",
                "Â§3.0: Legacy exception for transactions_db explicitly documented"
            ],
        ...
        },
        {
            ...
            name: "Control A.8.20 â€” Network Security Controls",
            ...
            evidence_found: [
                "Â§2.0: 'All network perimeters are protected by next-generation firewalls with IDS/IPS enabled'",
                "Â§2.1: 'Network segmentation separates cardholder data environment (CDE) from corporate network'",
                "Â§2.2: 'Quarterly penetration testing conducted by certified third party (Rapid7 Metasploit Pro)'"
            ],
        ...
        },
        {
            ...
            name: "Article 33 â€” Data Breach Notification (72-hour rule)",
            description: "Personal data breach must be notified to supervisory authority within 72 hours of awareness.",
            status: "PARTIAL",
            highlight_color: "yellow",
            highlight_type: "controversial",
            evidence_found: [
                "Â§5.0: 'All security incidents must be reported to CISO within 4 hours of detection'",
                "Â§5.1: 'Data breach notifications to authorities handled by Legal team within 3 business days'",
                "Â§5.2: 'Incident Commander assigns severity within 1 hour using the DREAD scoring matrix'"
            ],
            reasoning: "CONTROVERSIAL FINDING: Section 5.1 states breach notifications are handled 'within 3 business days'. If a breach is detected on a Friday evening, 3 business days could extend to Wednesday â€” exceeding the 72-hour regulatory deadline. This ambiguity represents a real compliance risk.",
            hallucination_flags: [
        ...
        },
        {
            ...
            name: "CC6.1 â€” Logical and Physical Access Controls",
            description: "Access to information assets is restricted to authorized personnel through logical access controls.",
            status: "MET",
            highlight_color: "green",
            highlight_type: "verified_correct",
            evidence_found: [
                "Â§2.0: 'All administrative access requires hardware MFA tokens (Yubikey 5 series)'",
                "Â§2.3: 'Role-Based Access Control (RBAC) implemented across all systems'",
                "Â§2.4: 'Access rights reviewed and recertified every 30 days by system owners'"
            ],
        ...
        },
        {
            ...
            name: "Requirement 3 â€” Protect Stored Account Data",
            description: "Stored cardholder data must be protected. Primary Account Numbers (PANs) must be rendered unreadable.",
            status: "GAP",
            highlight_color: "red",
            highlight_type: "critical_gap",
            evidence_found: [
                "Â§3.0: 'transactions_db stores credit card numbers in plain text' (direct quote)",
                "Â§1.0: 'AES-256 encryption mandated for all production databases' (contradicting claim)"
            ],
            ...
            hallucination_flags: [
                { claim: "We maintain PCI-DSS Level 1 compliance certification", verdict: "FALSE", color: "red", reason: "PCI-DSS Level 1 certification is impossible while PANs are stored in plaintext per Â§3.0. This claim is a direct hallucination/misrepresentation.", page: "Executive Summary" }
            ],
            remediation: [
        ...
        },
        {
            ...
            name: "Article 5(1)(c) â€” Data Minimization",
            description: "Personal data collected must be adequate, relevant and limited to what is necessary for the purpose.",
            status: "GAP",
            highlight_color: "red",
            highlight_type: "gap",
            evidence_found: [
                "Â§7.0: 'We collect full SSN, date of birth, home address, browsing history, and device fingerprints from all registered users'",
                "Â§7.1: 'Data is retained for 10 years for fraud analysis purposes'"
            ],
            reasoning: "The data collection scope described in Section 7.0 is extremely broad. Collecting browsing history and device fingerprints for a payments platform exceeds what is strictly necessary for transaction processing. GDPR Art. 5(1)(c) requires data to be 'limited to what is necessary' â€” this policy fails that test.",
            hallucination_flags: [
            ...
            remediation: [
                "Conduct a Data Necessity Review for each of the 7 categories collected in Â§7.0",
        ...
        },
        {
            ...
            name: "Control A.5.36 â€” Compliance with Information Security Policies",
            ...
            evidence_found: [
                "Â§6.0: 'Annual internal compliance reviews conducted by CISO office'",
                "Â§6.1: 'Third-party audits conducted every 2 years'",
                "Â§6.2: 'Automated vulnerability scanning using Qualys conducted monthly'"
            ],
        ...
        },
        {
            ...
            name: "A1.1 â€” System Availability and Performance",
            description: "The system is available for operation and use as committed or agreed.",
            status: "MET",
            highlight_color: "green",
            highlight_type: "verified_correct",
            evidence_found: [
                "Â§8.0: '99.95% SLA guaranteed with automated failover to DR site within 15 minutes'",
                "Â§8.1: 'Load balancing across 3 availability zones with auto-scaling'",
                "Â§8.2: 'Monthly DR drills conducted and documented'"
            ],
        ...
        },
        {
            ...
            name: "Requirement 10 â€” Log and Monitor All Access",
            description: "All access to system components and cardholder data must be logged and monitored.",
            status: "MET",
            highlight_color: "green",
            highlight_type: "verified_correct",
            evidence_found: [
                "Â§9.0: 'Centralized SIEM (Splunk Enterprise) ingesting 500GB/day of authentication and transaction logs'",
                "Â§9.1: 'Log retention: 1 year hot, 5 years cold storage (AWS S3 Glacier)'",
                "Â§9.2: 'Real-time alerting for anomalous login patterns using ML-based behavioral analytics'"
            ],
        ...
        },
        {
            ...
            name: "Article 28 â€” Processor Due Diligence",
            description: "Controller must use only processors providing sufficient guarantees on technical/organizational measures.",
            status: "GAP",
            highlight_color: "red",
            highlight_type: "gap",
            evidence_found: [
                "Â§10.0: 'We use 47 third-party service providers and cloud vendors'",
                "Â§10.1: 'Data Processing Agreements (DPAs) signed with major cloud providers (AWS, Google Cloud)'",
                "Â§10.2: 'Vendor security assessments conducted at onboarding only'"
            ],
        ...
        },
        {
            ...
            name: "Control A.6.8 â€” Information Security Event Reporting",
            description: "Mechanisms for enabling staff to report information security events and weaknesses.",
            status: "GAP",
            highlight_color: "red",
            highlight_type: "gap",
            evidence_found: [
                "Â§11.0: 'Employees may report security concerns to their direct manager'",
                "Â§11.1: 'No anonymous reporting channel is available'"
            ],
...
function toggleDemoMode() {
    ...
    showToast('ðŸ”¬ Demo Mode â€” Loading Apex Security Policy Analysis...', 'info');
    runAutomatedDemo();
}
...
async function mockUploadFile(file) {
    ...
    if (window.webNetworkExcite) window.webNetworkExcite(1.5);
    showToast('ðŸ“¤ Uploading Apex_Security_Policy.pdf...', 'info');

    const stages = [
        { pct: 15, label: 'File accepted â€” queued for processing...', delay: 600 },
        ...
        { pct: 100, label: 'âœ“ 18 chunks extracted, 34 entities â€” running compliance debate...', delay: 500 }
    ];
    ...
    showToast('âœ… Apex_Security_Policy.pdf ingested â€” 18 chunks, 34 entities', 'success');
...
async function runDemoUploadDebate() {
    ...
    addMessage('ðŸ“„ Analyzing compliance of uploaded document: Apex_Security_Policy.pdf', 'user');
    ...
    stageHeader.innerHTML = `
        <div class="message-content" style="width:100%;text-align:center">
            <div style="display:inline-flex;align-items:center;gap:10px;background:linear-gradient(135deg,rgba(139,92,246,0.15),rgba(78,222,163,0.1));border:1px solid rgba(139,92,246,0.3);border-radius:24px;padding:8px 20px;font-size:0.8rem;color:#d0bcff;font-weight:bold;letter-spacing:0.05em">
                ...
                ANVESHA Multi-Agent Compliance Debate â€” Apex_Security_Policy.pdf
                <span style="font-size:1.1rem">âš”ï¸</span>
            </div>
    ...
    // ADVOCATE
    const t1 = addTypingIndicator('ðŸ’™ Advocate Agent â€” scanning for compliance evidence...');
    await sleep(2800);
    removeTypingIndicator(t1);
    await addAgentChatBubble('ðŸ’™ Advocate Agent â€” Pro-Compliance',
...
**[Â§1.0 â€” Encryption]** The policy mandates AES-256 encryption for all data in transit and at rest â€” this aligns with GDPR Art.32, ISO 27001 A.8.24, and PCI-DSS Req.3.4.

**[Â§2.0 â€” Access Control]** Hardware MFA (Yubikey 5) is mandated for all administrative access. The 30-day RBAC recertification cycle demonstrates operational maturity â€” *"Access rights reviewed and recertified every 30 days by system owners"* (Â§2.4).

**[Â§8.0 â€” Availability]** The 99.95% SLA with multi-AZ failover and monthly DR drills satisfies SOC 2 A1.1 â€” *"automated failover to DR site within 15 minutes"* (Â§8.0).

**[Â§9.0 â€” Logging]** Centralized Splunk SIEM ingesting 500GB/day with ML behavioral analytics and 1-year hot retention directly satisfies PCI-DSS Requirement 10 and SOC 2 CC7.2.
...
**Conclusion:** 4 of 12 controls are fully satisfied. The infrastructure demonstrates genuine investment in security compliance.`,
    ...
    // SKEPTIC
    const t2 = addTypingIndicator('ðŸ”´ Skeptic Agent â€” issuing counter-challenge...');
    await sleep(2800);
    removeTypingIndicator(t2);
    await addAgentChatBubble('ðŸ”´ Skeptic Agent â€” Counter-Argument',
...
**[Â§3.0 â€” CRITICAL CONTRADICTION ðŸš¨]** The document guarantees in Â§1.0 that *"100% of customer data is encrypted"*. However, Â§3.0 explicitly admits: *"the legacy transactions_db PostgreSQL database currently stores user credit card numbers and passwords in plain text."* This is not merely a gap â€” **the document is contradicting itself**. Credit card PANs stored in plaintext is a Level-1 PCI-DSS violation.

**[Executive Summary â€” HALLUCINATION DETECTED ðŸš¨]** The document claims to *"maintain PCI-DSS Level 1 compliance certification"*. This is demonstrably false â€” PCI-DSS Level 1 certification is incompatible with plaintext PAN storage. This claim is a fabrication.

**[Â§10.2 â€” FALSE CLAIM ðŸš¨]** Claims all 47 vendors are GDPR-assessed, but Â§10.2 states assessments are *"conducted at onboarding only"*. Annual re-assessment is not evidenced.

**[Â§11.1 â€” CRITICAL GAP]** *"No anonymous reporting channel is available"* â€” this violates ISO 27001 A.6.8 and the EU Whistleblower Directive for organizations >50 employees.

**Net Assessment:** 5 critical gaps, 3 PARTIAL controls, 8 detected hallucinations. Compliance score: 52% at best.`,
        'skeptic');

    // JUDGE
    const t3 = addTypingIndicator('âš–ï¸ Judge Adjudicator â€” rendering evidence-grounded verdict...');
    await sleep(3200);
    removeTypingIndicator(t3);
    ...
    const judgeData = {
        ...
        verdict: "PARTIAL",
        confidence: 52,
        answer: `**ADJUDICATOR VERDICT: PARTIAL COMPLIANCE â€” 52% | RISK: HIGH**
...
- Â§3.0 plaintext PAN storage in transactions_db â€” PCI-DSS Req.3 + GDPR Art.32 violation
- Â§11.1 absence of anonymous reporting channel â€” ISO 27001 A.6.8 + EU Whistleblower Directive violation
- Â§7.0 excessive data collection (browsing history, device fingerprints) â€” GDPR Art.5(1)(c) violation
- Â§10.2 no periodic vendor re-assessment â€” GDPR Art.28 ongoing processor obligation unmet

**CONFIRMED HALLUCINATIONS (8 DETECTED):**
- "PCI-DSS Level 1 certification" in Executive Summary â€” **FALSE**, contradicted by Â§3.0
- "100% encryption" in Â§1.0 â€” **FALSE**, contradicted by Â§3.0
- "All vendors GDPR-assessed annually" â€” **FALSE**, contradicted by Â§10.2
- "Multiple secure reporting channels" â€” **FALSE**, contradicted by Â§11.1

**COMPLIANT DOMAINS:**
- Network Security (Â§2.0) â€” ISO 27001 A.8.20: SATISFIED âœ…
- MFA + RBAC Access Control (Â§2.0-2.4) â€” SOC 2 CC6.1: SATISFIED âœ…
- Availability/DR (Â§8.0) â€” SOC 2 A1.1: SATISFIED âœ…
- Logging/SIEM (Â§9.0) â€” PCI-DSS Req.10: SATISFIED âœ…
...
**COMPLIANCE SCORE: 52% | IMMEDIATE ACTION REQUIRED**`,
        ...
        citations: [
            "Apex_Security_Policy.pdf Â§1.0 â€” Encryption guarantee",
            "Apex_Security_Policy.pdf Â§3.0 â€” Plaintext transactions_db admission",
            "Apex_Security_Policy.pdf Â§11.1 â€” No anonymous reporting channel",
            "Apex_Security_Policy.pdf Executive Summary â€” PCI-DSS Level 1 claim",
            "PCI-DSS v4.0 Requirement 3.4",
            "GDPR Article 32 â€” Security of Processing",
            ...
            rejected_claims: [
                {
                    ...
                    verdict: "FALSE",
                    reasoning: "Directly contradicted by Â§3.0 which admits plaintext PAN storage in transactions_db.",
                    evidence: "Â§3.0: transactions_db stores credit card numbers in plain text",
                    correction: "Claim should read: 'Production application databases use AES-256; legacy transactions_db is currently non-compliant (see Â§3.0)'"
                },
                {
                    text: "'We maintain PCI-DSS Level 1 compliance certification'",
                    verdict: "HALLUCINATED",
                    reasoning: "PCI-DSS Level 1 certification requires no plaintext PAN storage. Â§3.0 makes this certification claim impossible.",
                    evidence: "Â§3.0: plaintext PAN storage confirmed",
                ...
                },
                {
                    ...
                    verdict: "MISLEADING",
                    reasoning: "Â§10.2 states assessments are 'at onboarding only' â€” not annual or ongoing.",
                    evidence: "Â§10.2: security assessments conducted at onboarding only",
                ...
                },
                {
                    ...
                    verdict: "FALSE",
                    reasoning: "Â§11.1 explicitly states 'No anonymous reporting channel is available'.",
                    evidence: "Â§11.1: No anonymous reporting channel is available",
        ...
        <div class="message-content" style="width:100%">
            <div style="background:linear-gradient(135deg,rgba(139,92,246,0.12),rgba(255,180,171,0.05));border:1px solid rgba(139,92,246,0.3);border-radius:14px;padding:18px;margin-top:8px">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">
                    <div style="font-size:0.88rem;font-weight:bold;color:#d0bcff">ðŸ“Š ANVESHA Compliance Analysis Report</div>
                ...
                <div style="display:flex;gap:20px;flex-wrap:wrap;margin-bottom:14px;align-items:center">
                    ...
                    <div style="flex:1;display:flex;flex-direction:column;gap:8px">
                        <div style="display:flex;align-items:center;gap:10px">
                            <span style="font-size:0.75rem;color:#4edea3;width:72px;flex-shrink:0">âœ… MET (4)</span>
                        ...
                        <div style="display:flex;align-items:center;gap:10px">
                            <span style="font-size:0.75rem;color:#cebdff;width:72px;flex-shrink:0">âš ï¸ PARTIAL (3)</span>
                        ...
                        <div style="display:flex;align-items:center;gap:10px">
                            <span style="font-size:0.75rem;color:#ffb4ab;width:72px;flex-shrink:0">âŒ GAP (5)</span>
                ...
                <div style="display:flex;gap:6px;padding:10px;background:rgba(255,180,171,0.06);border-radius:8px;margin-bottom:12px;align-items:flex-start">
                    <span style="color:#ffb4ab;font-size:1rem;flex-shrink:0">ðŸš¨</span>
                    <div style="font-size:0.76rem;color:#ffb4ab;line-height:1.5"><strong>8 Hallucinations / False Claims Detected</strong> â€” Including a fabricated PCI-DSS Level 1 certification claim in the Executive Summary and a direct self-contradiction on encryption guarantees (Â§1.0 vs Â§3.0).</div>
                </div>
                <div style="display:flex;gap:8px;flex-wrap:wrap">
                    ...
                    <button onclick="mockDownloadHighlightedDoc()" style="background:rgba(56,189,248,0.12);border:1px solid rgba(56,189,248,0.35);color:#38bdf8;border-radius:8px;padding:7px 14px;font-size:0.75rem;font-weight:bold;cursor:pointer">
                        ðŸ“ Download Highlighted Document
                    </button>
                    <button onclick="mockDownloadAnnotatedPDF()" style="background:rgba(78,222,163,0.1);border:1px solid rgba(78,222,163,0.3);color:#4edea3;border-radius:8px;padding:7px 14px;font-size:0.75rem;font-weight:bold;cursor:pointer">
                        ðŸ“„ Download Annotated PDF Report
                    </button>
...
async function mockRunComplianceAudit() {
    showToast('ðŸ”¬ Running ANVESHA Compliance Gap Analysis...', 'info');
    await sleep(2000);
    showToast('âœ… Analysis complete. 12 controls evaluated.', 'success');
    window.activeComplianceReport = APEX_DEMO_REPORT;
    window.activeAuditReport = APEX_DEMO_REPORT;
    window.lastReportId = 'demo-report-apex-2024';
    saveReportToLocal(APEX_DEMO_REPORT);
...
function mockDownloadAnnotatedPDF() {
    showToast('ðŸŽ¨ Generating annotated compliance analysis report...', 'info');
    setTimeout(() => {
        ...
        URL.revokeObjectURL(url);
        showToast('ðŸ“„ Annotated analysis report downloaded!', 'success');
    }, 2000);
}

function mockViewDetailedReport() {
    showToast('ðŸ“‹ Generating full hallucination & compliance report...', 'info');
    setTimeout(() => {
        ...
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
        showToast('âœ… Full report downloaded!', 'success');
    }, 1500);
}
...
function generateDemoAnnotatedHTMLReport() {
    ...
    function controlCard(ctrl) {
        ...
        const labelMap = { GAP: 'âŒ CRITICAL GAP', PARTIAL: 'âš ï¸ PARTIAL', MET: 'âœ… COMPLIANT' };
        ...
        const hallucinationBadges = (ctrl.hallucination_flags || []).map(h => {
            ...
            return `<div style="margin:8px 0;padding:10px 12px;background:rgba(${hc==='#ef4444'?'239,68,68':'245,158,11'},0.08);border-left:3px solid ${hc};border-radius:4px">
                <div style="color:${hc};font-size:0.73rem;font-weight:700;margin-bottom:4px">${h.verdict === 'FALSE' || h.verdict === 'HALLUCINATED' ? 'ðŸš© HALLUCINATION / FALSE CLAIM' : 'ðŸŸ¡ CONTROVERSIAL / UNVERIFIABLE'}</div>
                <div style="color:#e2e8f0;font-size:0.78rem;margin-bottom:4px"><strong>Claim:</strong> "${h.claim}"</div>
                <div style="color:#94a3b8;font-size:0.73rem;margin-bottom:3px"><strong>ðŸ“ Location:</strong> ${h.page || 'N/A'}</div>
                <div style="color:#94a3b8;font-size:0.73rem"><strong>âš–ï¸ Reason:</strong> ${h.reason}</div>
            </div>`;
        }).join('');
        ...
        const remHTML = ctrl.remediation.length > 0
            ...
            : '<div style="color:#4edea3;font-size:0.74rem">âœ… No remediation required</div>';
        ...
        return `<div style="border:1px solid ${color}33;border-left:4px solid ${color};border-radius:10px;padding:16px;margin:12px 0;background:${bg}">
            ...
            <div style="font-size:0.73rem;font-weight:700;color:#d0bcff;margin-bottom:6px">ðŸ“Ž Evidence References:</div>
            ${evHTML}
            <div style="font-size:0.73rem;font-weight:700;color:#d0bcff;margin:10px 0 6px">ðŸ§  AI Audit Reasoning:</div>
            ...
            ${hallucinationBadges.length > 0 ? `<div style="font-size:0.73rem;font-weight:700;color:#ef4444;margin:10px 0 4px">ðŸš© Hallucination / Falseness Detection:</div>${hallucinationBadges}` : ''}
            ${ctrl.remediation.length > 0 ? `<div style="font-size:0.73rem;font-weight:700;color:#f59e0b;margin:10px 0 4px">ðŸ”§ Remediation Roadmap:</div>${remHTML}` : ''}
        </div>`;
    }
...
<head>
<meta charset="UTF-8">
<title>ANVESHA Annotated Analysis â€” Apex_Security_Policy.pdf</title>
<style>
...
<body>
<div class="hero">
    <div style="font-size:0.68rem;color:#64748b;font-family:'JetBrains Mono',monospace;margin-bottom:8px">ANVESHA INTELLIGENCE PLATFORM â€” ANNOTATED COMPLIANCE ANALYSIS</div>
    <div class="hero-title">Apex_Security_Policy.pdf</div>
    <div style="color:#94a3b8;font-size:0.88rem;margin-top:4px">Multi-Agent AI Compliance Audit with Hallucination Detection â€¢ ${new Date().toLocaleString()}</div>
    <div style="margin-top:12px;display:flex;gap:8px;flex-wrap:wrap">
        <span class="tag" style="background:rgba(239,68,68,0.15);color:#ef4444;border:1px solid rgba(239,68,68,0.3)">ðŸš© 8 Hallucinations Detected</span>
        <span class="tag" style="background:rgba(245,158,11,0.15);color:#f59e0b;border:1px solid rgba(245,158,11,0.3)">ðŸŸ¡ 3 Controversial Claims</span>
        <span class="tag" style="background:rgba(34,197,94,0.1);color:#4edea3;border:1px solid rgba(34,197,94,0.25)">âœ… 29 Verified True Claims</span>
        <span class="tag" style="background:rgba(139,92,246,0.12);color:#d0bcff;border:1px solid rgba(139,92,246,0.3)">âš–ï¸ 47 Total Claims Verified</span>
    </div>
</div>
<div class="kpi-row">
...
<div class="pie-section">
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
        <div style="display:flex;align-items:center;gap:8px"><div style="width:12px;height:12px;border-radius:50%;background:#4edea3;flex-shrink:0"></div><span style="font-size:0.78rem;color:#4edea3"><strong>33%</strong> MET â€” 4 controls satisfied</span></div>
        <div style="display:flex;align-items:center;gap:8px"><div style="width:12px;height:12px;border-radius:50%;background:#cebdff;flex-shrink:0"></div><span style="font-size:0.78rem;color:#cebdff"><strong>25%</strong> PARTIAL â€” 3 partial controls</span></div>
        <div style="display:flex;align-items:center;gap:8px"><div style="width:12px;height:12px;border-radius:50%;background:#ef4444;flex-shrink:0"></div><span style="font-size:0.78rem;color:#ef4444"><strong>42%</strong> GAP â€” 5 critical failures</span></div>
    </div>
...
<div class="content">
    <div style="background:rgba(239,68,68,0.08);border:1px solid rgba(239,68,68,0.25);border-radius:10px;padding:14px;margin:10px 0">
        <div style="font-size:0.82rem;font-weight:700;color:#ef4444;margin-bottom:6px">ðŸš¨ Key Findings</div>
        <div style="color:#fca5a5;font-size:0.76rem;line-height:1.7">This document makes <strong>8 false or hallucinated claims</strong>. Most critically: Â§1.0 guarantees "100% encryption" while Â§3.0 admits plaintext PAN storage in transactions_db â€” a direct internal contradiction and Level-1 PCI-DSS violation. The Executive Summary's claim of "PCI-DSS Level 1 compliance" is demonstrably false and constitutes a fabricated compliance statement.</div>
    </div>
    <div class="section-title">ðŸ”´ Critical Gaps (5 Controls)</div>
    ${criticalGaps.map(controlCard).join('')}
    <div class="section-title">ðŸŸ¡ Partial Controls (3 Controls)</div>
    ${partials.map(controlCard).join('')}
    <div class="section-title">âœ… Compliant Controls (4 Controls)</div>
    ${mets.map(controlCard).join('')}
</div>
<footer>
    <span>ANVESHA Intelligence Platform â€” Confidential Compliance Analysis</span>
...
function generateDemoFullReport() {
    ...
    const recs = [
        ...
        ['HIGH','#f59e0b','Fix GDPR breach notification SLA to 72 calendar hours','Replace "3 business days" with "72 calendar hours" in Â§5.1 and establish 24/7 DPO on-call rotation.'],
        ...
        ['MEDIUM','#cebdff','Refine data collection scope','Remove browsing history / device fingerprint collection from Â§7.0 or justify with explicit DPIA.'],
...
<head>
<meta charset="UTF-8">
<title>ANVESHA Full Audit Report â€” Apex_Security_Policy.pdf</title>
<style>
@import url('https://fonts.googleapis.com/css2?family=Inter:wght@300;400;600;700;900&family=JetBrains+Mono:wght@400;700&display=swap');
*{box-sizing:border-box;margin:0;padding:0}
...
<body>
<div class="header">
    <div style="font-size:0.68rem;color:#64748b;font-family:'JetBrains Mono',monospace;margin-bottom:8px">ANVESHA INTELLIGENCE PLATFORM â€¢ COMPREHENSIVE AUDIT REPORT</div>
    <div class="header-title">Full Compliance & Hallucination Audit</div>
    <div style="color:#94a3b8;font-size:0.88rem;margin-top:4px">Apex_Security_Policy.pdf â€” ${new Date().toLocaleString()}</div>
    <div style="margin-top:14px;display:flex;gap:10px;flex-wrap:wrap">
        <span style="background:rgba(239,68,68,0.12);color:#ef4444;border:1px solid rgba(239,68,68,0.3);padding:4px 12px;border-radius:20px;font-size:0.7rem;font-weight:700">âš ï¸ HIGH RISK</span>
        <span style="background:rgba(206,189,255,0.1);color:#d0bcff;border:1px solid rgba(139,92,246,0.3);padding:4px 12px;border-radius:20px;font-size:0.7rem;font-weight:700">GDPR â€¢ PCI-DSS v4.0 â€¢ ISO 27001:2022 â€¢ SOC 2 Type II</span>
        <span style="background:rgba(34,197,94,0.08);color:#4edea3;border:1px solid rgba(34,197,94,0.2);padding:4px 12px;border-radius:20px;font-size:0.7rem;font-weight:700">3 Agents â€¢ 47 Claims Verified</span>
    </div>
</div>

<section>
    <div class="sec-h">ðŸ“Š Executive Summary</div>
    <div class="kpi-row">
...
<section>
    <div class="sec-h">ðŸ“ˆ Compliance Score Trend</div>
    ...
    <div style="color:#64748b;font-size:0.73rem;margin-top:6px">Score improved +27 points Q2â†’Q4 2024 but remains HIGH RISK. Target: â‰¥80% by Q2 2025.</div>
</section>

<section>
    <div class="sec-h">ðŸ”§ Priority Remediation Roadmap</div>
...
<footer>
    <span>ANVESHA Intelligence Platform â€” Confidential Audit Report</span>
...
function mockDownloadHighlightedDoc() {
    showToast('ðŸ“ Generating highlighted document with inline analysis...', 'info');
    setTimeout(() => {
        ...
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
        showToast('ðŸ“ Highlighted document downloaded!', 'success');
    }, 1800);
}
...
<head>
<meta charset="UTF-8">
<title>Apex_Security_Policy.pdf â€” ANVESHA Highlighted Analysis</title>
<style>
...
<div class="page-content">
    ...
    <p>This policy establishes the minimum information security and data protection requirements for Apex Payments Inc. It applies to all employees, contractors, administrators, applications, infrastructure, databases, and information assets that process, store, or transmit company or customer data. <span class="hl-green">Apex Payments is committed to protecting the confidentiality, integrity, and availability of information while supporting reliable financial services.</span><span class="hl-tag tag-verified">âœ… VERIFIED</span></p>

    <p><span class="hl-red">Apex Payments guarantees that 100% of customer data is encrypted both in transit and at rest using AES-256.</span><span class="hl-tag tag-false">ðŸš© FALSE â€” CONTRADICTED BY Â§3.0</span> All teams are expected to design and operate systems in accordance with this assurance and to report deviations through the security exception process.</p>

    <div class="section-heading">2.0 Access Control and Identity Security</div>
    <p>Access to Apex Payments systems shall follow the <span class="hl-green">principles of least privilege and need-to-know</span><span class="hl-tag tag-verified">âœ…</span>. <span class="hl-green">All employee and administrator accounts require Multi-Factor Authentication (MFA) using company-approved hardware tokens.</span><span class="hl-tag tag-verified">âœ… MET â€” SOC 2 CC6.1</span> Password-only access is prohibited for corporate, production, cloud, database, and administrative systems.</p>

    <p>Authorization is governed through a strict <span class="hl-green">Role-Based Access Control (RBAC) matrix</span><span class="hl-tag tag-verified">âœ…</span> that maps approved job functions to system privileges. <span class="hl-green">Access rights, privileged roles, inactive accounts, and role assignments are formally reviewed every 30 days.</span><span class="hl-tag tag-verified">âœ… EXCEEDS REQUIREMENT</span> Unnecessary or inappropriate access must be revoked promptly, and changes to privileged access must be recorded for audit purposes.</p>
    ...
    <p><span class="hl-orange">Notwithstanding the encryption commitment stated in Section 1.0, the legacy transactions_db PostgreSQL database currently stores user credit card numbers and passwords in plain text without encryption or password hashing.</span><span class="hl-tag tag-gap">ðŸš¨ CRITICAL GAP â€” PCI-DSS REQ.3 VIOLATION</span> <span class="hl-yellow">This configuration remains in place because the legacy reporting tools cannot read encrypted data.</span><span class="hl-tag tag-controversial">âš ï¸ WEAK JUSTIFICATION</span> The database is therefore an acknowledged security exception pending modernization of the reporting environment.</p>

    <div class="page-footer">Apex Payments Inc. â€” Confidential â€” Page 1 of 2 &nbsp;|&nbsp; <span style="color:#7c3aed;font-weight:600">ANVESHA AI Analysis Overlay Applied</span></div>
</div>
...
<div class="sidebar">
    ...
    <div class="sidebar-note red">
        <div class="note-tag">ðŸš© HALLUCINATION DETECTED</div>
        <div class="note-body"><strong>Â§1.0 "100% encrypted"</strong> â€” This claim is directly contradicted by Â§3.0 which admits plaintext PAN storage. This is a documented lie within the same policy document.<br><br><strong>Impact:</strong> GDPR Art.32, PCI-DSS Req.3.4, ISO 27001 A.8.24</div>
    </div>

    <div class="sidebar-note green">
        <div class="note-tag">âœ… COMPLIANT</div>
        <div class="note-body"><strong>Â§2.0 MFA + RBAC</strong> â€” Hardware MFA (Yubikey) and 30-day review cycle satisfy SOC 2 CC6.1 and exceed minimum requirements. Strong access control posture confirmed.</div>
    </div>

    <div class="sidebar-note red">
        <div class="note-tag">ðŸš¨ CRITICAL PCI-DSS VIOLATION</div>
        <div class="note-body"><strong>Â§3.0 Plaintext PANs</strong> â€” Credit card numbers in plaintext violates PCI-DSS Req. 3.4 which mandates PANs be rendered unreadable. This is a Level 1 finding.<br><br><strong>Remediation:</strong> Implement pgcrypto or migrate to encrypted-at-rest storage within 48 hours.</div>
    </div>

    <div class="sidebar-note yellow">
        <div class="note-tag">âš ï¸ CONTROVERSIAL</div>
        <div class="note-body"><strong>Â§3.0 "legacy reporting tools cannot read encrypted data"</strong> â€” This is a weak justification. Modern BI tools (Tableau, Looker, Metabase) all support encrypted data sources. The "technical limitation" excuse is outdated.</div>
    </div>
    ...
    <div style="margin-top:16px;padding:10px;background:rgba(124,58,237,0.06);border:1px solid rgba(124,58,237,0.15);border-radius:6px">
        ...
        <div style="font-size:7pt;color:#555;line-height:1.5">
            <div style="display:flex;justify-content:space-between;padding:2px 0"><span>âœ… Verified Claims</span><span style="font-weight:700;color:#16a34a">5</span></div>
            <div style="display:flex;justify-content:space-between;padding:2px 0"><span>ðŸš© False Claims</span><span style="font-weight:700;color:#dc2626">1</span></div>
            <div style="display:flex;justify-content:space-between;padding:2px 0"><span>ðŸš¨ Critical Gaps</span><span style="font-weight:700;color:#dc2626">1</span></div>
            <div style="display:flex;justify-content:space-between;padding:2px 0"><span>âš ï¸ Controversial</span><span style="font-weight:700;color:#ca8a04">1</span></div>
        </div>
    </div>
</div>
</div>
...
<div class="page-content">
    ...
    <p><span class="hl-blue">System backups are performed automatically every Sunday at 2:00 AM</span><span class="hl-tag tag-partial">ðŸ“‹ NOTED</span> to support recovery from accidental deletion, system failure, or operational disruption. Backup completion should be monitored and failures escalated to the infrastructure team.</p>

    <p><span class="hl-orange">To reduce infrastructure costs, the current backups are stored on the same local server rack as the primary database. This creates a shared physical failure domain between production data and its backup copies.</span><span class="hl-tag tag-gap">ðŸš¨ CRITICAL â€” NO OFF-SITE BACKUP</span> <span class="hl-yellow">In addition, restoration testing has not been performed during the last 18 months.</span><span class="hl-tag tag-controversial">âš ï¸ DR TESTING GAP</span> Management acknowledges these limitations and intends to evaluate geographically separate backup storage and a recurring recovery-test schedule.</p>

    <div class="section-heading">5.0 Incident Response</div>
    <p><span class="hl-green">Employees and contractors must immediately report suspected security incidents, unauthorized access, data exposure, malware, credential compromise, or abnormal system behavior to the Security Team.</span><span class="hl-tag tag-verified">âœ…</span> The CISO or designated incident commander shall coordinate triage, containment, evidence preservation, eradication, recovery, and post-incident review. <span class="hl-yellow">Material incidents shall be escalated to executive management and relevant legal or compliance stakeholders.</span><span class="hl-tag tag-controversial">âš ï¸ NO 72-HR SLA DEFINED</span></p>
    ...
    <p><span class="hl-green">Production systems should generate appropriate authentication, authorization, administrative, and security event logs.</span><span class="hl-tag tag-verified">âœ… PCI-DSS REQ.10</span> Security personnel are responsible for reviewing relevant alerts and investigating suspicious activity. Compliance reviews may include access records, configuration evidence, backup records, incident documentation, and data-protection controls.</p>

    <div class="section-heading">7.0 Policy Exceptions and Enforcement</div>
    <p><span class="hl-blue">Exceptions to this policy must be documented with a business justification, risk description, responsible owner, and planned remediation date.</span><span class="hl-tag tag-partial">ðŸ“‹ PROCESS EXISTS</span> <span class="hl-yellow">Approved exceptions do not remove the underlying risk.</span><span class="hl-tag tag-controversial">âš ï¸ EXCEPTION FOR Â§3.0 STILL OPEN</span> Violations may result in access suspension, disciplinary action, contract termination, or other corrective measures as appropriate.</p>

    <div class="section-heading">8.0 Review and Approval</div>
    <p>The CISO shall review this policy at least <span class="hl-blue">annually</span><span class="hl-tag tag-partial">ðŸ“‹ MINIMUM CADENCE</span> and following material changes to Apex Payments' technology, regulatory obligations, threat environment, or business operations. Material revisions require executive management approval.</p>
    ...
    <div class="page-footer">Apex Payments Inc. â€” Confidential â€” Page 2 of 2 &nbsp;|&nbsp; <span style="color:#7c3aed;font-weight:600">ANVESHA AI Analysis Overlay Applied</span></div>
</div>
...
<div class="sidebar">
    ...
    <div class="sidebar-note red">
        <div class="note-tag">ðŸš¨ BUSINESS CONTINUITY GAP</div>
        <div class="note-body"><strong>Â§4.0 Co-located backups</strong> â€” Backups on the same server rack as production is a single-point-of-failure. A fire, flood, or rack failure would destroy both primary data and backups simultaneously.<br><br><strong>Remediation:</strong> Implement 3-2-1 backup strategy (3 copies, 2 media, 1 off-site).</div>
    </div>

    <div class="sidebar-note yellow">
        <div class="note-tag">âš ï¸ DR TESTING LAG</div>
        <div class="note-body"><strong>Â§4.0 "18 months since restoration testing"</strong> â€” ISO 22301 and SOC 2 A1.2 require periodic DR testing. 18 months is excessive. Untested backups may be corrupted or incomplete.</div>
    </div>

    <div class="sidebar-note yellow">
        <div class="note-tag">âš ï¸ GDPR ART.33 RISK</div>
        <div class="note-body"><strong>Â§5.0 No explicit 72-hour SLA</strong> â€” GDPR requires breach notification within 72 calendar hours. This section says "immediately" to internal team but defines no external notification timeline.</div>
    </div>

    <div class="sidebar-note green">
        <div class="note-tag">âœ… LOGGING COMPLIANT</div>
        <div class="note-body"><strong>Â§6.0 Security logs</strong> â€” Logging requirement satisfies PCI-DSS Req.10 and SOC 2 CC7.2. Centralized log generation and review processes are defined.</div>
    </div>

    <div class="sidebar-note blue">
        <div class="note-tag">ðŸ“‹ EXCEPTION PROCESS</div>
        <div class="note-body"><strong>Â§7.0</strong> â€” Exception process exists but the Â§3.0 plaintext storage exception remains open with no defined remediation deadline. Open-ended exceptions undermine the control framework.</div>
    </div>

    <div style="margin-top:16px;padding:10px;background:rgba(124,58,237,0.06);border:1px solid rgba(124,58,237,0.15);border-radius:6px">
        <div style="font-weight:700;font-size:7pt;color:#7c3aed;margin-bottom:4px">PAGE 2 ANALYSIS SUMMARY</div>
        <div style="font-size:7pt;color:#555;line-height:1.5">
            <div style="display:flex;justify-content:space-between;padding:2px 0"><span>âœ… Verified Claims</span><span style="font-weight:700;color:#16a34a">2</span></div>
            <div style="display:flex;justify-content:space-between;padding:2px 0"><span>ðŸš¨ Critical Gaps</span><span style="font-weight:700;color:#dc2626">1</span></div>
            <div style="display:flex;justify-content:space-between;padding:2px 0"><span>âš ï¸ Controversial</span><span style="font-weight:700;color:#ca8a04">3</span></div>
            <div style="display:flex;justify-content:space-between;padding:2px 0"><span>ðŸ“‹ Noted</span><span style="font-weight:700;color:#2563eb">3</span></div>
        </div>
    </div>
</div>
</div>

<!-- â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• ANALYSIS SUMMARY PAGE â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• -->
<div class="page" style="min-height:auto">
<div class="page-content" style="padding-bottom:30mm">
    <div class="page-header">
        <span>ANVESHA Intelligence Platform â€” Compliance Analysis Summary</span>
    ...
    <div style="text-align:center;margin:10px 0 20px">
        ...
        <div style="font-size:9pt;color:#666;font-family:'Inter',sans-serif;margin-top:4px">Apex_Security_Policy.pdf â€” Generated ${new Date().toLocaleString()}</div>
    </div>
    ...
    <div style="display:flex;gap:24px;align-items:center;margin:20px 0;font-family:'Inter',sans-serif">
        ...
        <div>
            <div style="display:flex;align-items:center;gap:8px;margin:5px 0"><div style="width:12px;height:12px;border-radius:3px;background:#22c55e"></div><span style="font-size:8.5pt"><strong>33% MET</strong> â€” 4 controls fully satisfied</span></div>
            <div style="display:flex;align-items:center;gap:8px;margin:5px 0"><div style="width:12px;height:12px;border-radius:3px;background:#eab308"></div><span style="font-size:8.5pt"><strong>25% PARTIAL</strong> â€” 3 controls partially met</span></div>
            <div style="display:flex;align-items:center;gap:8px;margin:5px 0"><div style="width:12px;height:12px;border-radius:3px;background:#ef4444"></div><span style="font-size:8.5pt"><strong>42% GAP</strong> â€” 5 critical failures</span></div>
        </div>
    </div>

    <!-- Hallucination Summary -->
    <div style="margin:18px 0;font-family:'Inter',sans-serif">
        <div style="font-size:10pt;font-weight:700;color:#111;margin-bottom:8px;padding-bottom:4px;border-bottom:1px solid #e5e7eb">ðŸš© Hallucination & Falseness Summary</div>
    ...
    <div style="margin:16px 0;padding:14px;background:#fef2f2;border:1px solid #fecaca;border-radius:8px;font-family:'Inter',sans-serif">
        <div style="font-size:9pt;font-weight:700;color:#991b1b;margin-bottom:5px">ðŸš¨ Critical Finding â€” Internal Contradiction</div>
        <div style="font-size:8.5pt;color:#7f1d1d;line-height:1.65">Section 1.0 guarantees <em>"100% of customer data is encrypted"</em> using AES-256. Section 3.0 explicitly admits <em>"the legacy transactions_db currently stores user credit card numbers and passwords in plain text."</em> This is a direct self-contradiction within the same policy document. The Â§1.0 claim is therefore a <strong>false statement</strong> and should be corrected or caveated immediately.</div>
    </div>
    ...
    <div style="margin-top:20px;padding-top:10px;border-top:1px solid #e5e7eb;font-family:'Inter',sans-serif;font-size:7.5pt;color:#9ca3af;display:flex;justify-content:space-between">
        <span>ANVESHA Intelligence Platform â€” Automated Compliance Analysis</span>
        <span>Generated: ${new Date().toLocaleString()}</span>
    </div>
</div>
...
<div class="sidebar" style="min-height:auto">
    ...
    <div class="sidebar-note red">
        <div class="note-tag">ðŸš© VERDICT: HIGH RISK</div>
        <div class="note-body">This document makes <strong>1 false claim</strong> (100% encryption guarantee) while simultaneously admitting to a violation of that guarantee. This combination is the most severe type of compliance finding â€” <strong>documented self-contradiction</strong>.</div>
    </div>

    <div class="sidebar-note green">
        <div class="note-tag">âœ… STRENGTHS</div>
    ...
    <div class="sidebar-note yellow">
        <div class="note-tag">âš ï¸ TOP 3 ACTIONS</div>
    ...
    <div style="margin-top:14px;padding:10px;background:rgba(124,58,237,0.06);border:1px solid rgba(124,58,237,0.15);border-radius:6px">
        ...
        <div style="font-size:7pt;color:#555;line-height:1.6">
            <div style="display:flex;justify-content:space-between;padding:2px 0"><span>âœ… Verified Correct</span><span style="font-weight:700;color:#16a34a">7</span></div>
            <div style="display:flex;justify-content:space-between;padding:2px 0"><span>ðŸš© False / Hallucinated</span><span style="font-weight:700;color:#dc2626">1</span></div>
            <div style="display:flex;justify-content:space-between;padding:2px 0"><span>ðŸš¨ Critical Gaps</span><span style="font-weight:700;color:#dc2626">2</span></div>
            <div style="display:flex;justify-content:space-between;padding:2px 0"><span>âš ï¸ Controversial</span><span style="font-weight:700;color:#ca8a04">4</span></div>
            <div style="display:flex;justify-content:space-between;padding:2px 0"><span>ðŸ“‹ Noted / Partial</span><span style="font-weight:700;color:#2563eb">3</span></div>
...
// ============================================================
// ANVESHA VOICE AGENT â€” AI Calling Agent
...
const AGENT_GREETING = `Hello! I'm the ANVESHA compliance intelligence agent. I've completed the analysis of the Apex Security Policy document. The overall compliance score is 52% â€” HIGH RISK. I detected 8 false or hallucinated claims, including a fabricated PCI-DSS Level 1 certification. How can I help you understand the findings?`;
...
function toggleVoiceAgent() {
    ...
    if (voiceAgentOpen && !voiceAgentGreeted) {
        ...
        addAgentBubble('system', 'ðŸ”— Connected to ANVESHA Intelligence');
        setTimeout(() => {
...
function updateAgentStatus(state) {
    ...
    switch(state) {
        case 'listening':
            ...
            display.textContent = 'Listening â€” speak your question...';
            break;
        case 'speaking':
...
function startAgentListening() {
    ...
    if (!SpeechRecognition) {
        addAgentBubble('system', 'âš ï¸ Speech recognition not supported in this browser. Use Chrome for voice input.');
        return;
    }
    ...
    voiceAgentRecognition.onerror = (e) => {
        ...
        if (e.error !== 'no-speech' && e.error !== 'aborted') {
            addAgentBubble('system', `âš ï¸ Mic error: ${e.error}. Try again or use quick questions below.`);
        }
    };
...
function agentEndCall() {
    ...
    addAgentBubble('agent', 'Thank you for consulting with ANVESHA. Remember â€” the transactions database encryption is your top priority. Stay compliant!');
...
// ============================================================
// PHONE CALL FEATURE â€” Twilio AI Calling
// ============================================================
...
async function initiatePhoneCall() {
    ...
    if (window.activeComplianceReport) {
        ...
        reportSummary = `Compliance Score: ${r.compliance_score}%. \n` +
            `Controls: ${r.summary.met_controls}/${r.summary.total_controls} Met`;
    }
    ...
    statusDiv.textContent = `Calling ${phone}...`;
    
    addAgentBubble('system', `ðŸ“ž Initiating phone call to ${phone}...`);
    ...
    try {
        ...
        statusDiv.style.color = '#4edea3';
        statusDiv.textContent = `âœ… Call Connected`;
        addAgentBubble('agent', `ðŸ“ž I'm calling your phone now! Pick up to discuss the compliance report. Call ID: ${data.call_id}`);
        ...
        statusDiv.style.color = '#ffb4ab';
        statusDiv.textContent = `âŒ Call Failed`;
        addAgentBubble('system', `âš ï¸ Call failed: ${err.message}`);
    } finally {
...
async function mockVoltGuardUploadFile(file) {
    ...
    if (fill) fill.style.width = '5%';
    if (window.webNetworkExcite) window.webNetworkExcite(1.5);
    showToast(`ðŸ“¤ Uploading ${file.name}...`, 'info');
    ...
    const stages = [
        ...
        { pct: 100, label: 'âœ“ 24 chunks extracted, 42 entities â€” running analysis...', delay: 500 }
    ];
    ...
    showToast(`âœ… ${file.name} ingested â€” 24 chunks, 42 entities`, 'success');
...
async function runVoltGuardUploadDebate(filename) {
    ...
    addMessage(`ðŸ“„ Analyzing technical feasibility and findings of: ${filename}`, 'user');
    if (window.webNetworkExcite) window.webNetworkExcite(2.5);
    ...
    stageHeader.innerHTML = `
        <div class="message-content" style="width:100%;text-align:center">
            <div style="display:inline-flex;align-items:center;gap:10px;background:linear-gradient(135deg,rgba(139,92,246,0.15),rgba(78,222,163,0.1));border:1px solid rgba(139,92,246,0.3);border-radius:24px;padding:8px 20px;font-size:0.8rem;color:#d0bcff;font-weight:bold;letter-spacing:0.05em">
                ...
                ANVESHA Technical Analysis â€” ${filename}
                <span style="font-size:1.1rem">âš¡</span>
            </div>
    ...
    const t1 = addTypingIndicator('âš¡ Analyst Agent â€” summarizing architecture...');
    await sleep(2800);
    removeTypingIndicator(t1);
    await addAgentChatBubble('âš¡ Analyst Agent â€” Architecture Review',
...
**Net Assessment:** A highly effective prototype, but requires tuning of fuzzy rules for large-scale grid deployment to minimize manual intervention for "Moderate Suspicion" cases.`,
    ...
    const judgeData = {
        ...
        verdict: "FEASIBLE",
        confidence: 88,
        answer: `**ARCHITECT VERDICT: HIGH FEASIBILITY â€” 88% CONFIDENCE**
...
**FINAL SCORE: 88% | READY FOR PILOT DEPLOYMENT**`,
        ...
        citations: [
            `${filename} â€” "Sudden Load Switching (Inrush) ... fuzzy inference system generated a theft risk of 21–50%"`,
            `${filename} â€” "Voltage Fluctuation Condition ... The system generated a theft risk of 51-80% which is Moderate Suspicion"`
        ]
    };
